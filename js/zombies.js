...
function zombie(x,y){if(!blocked(x,y,13))zombies.push({id:uid++,x,y,hp:105+Math.min(t*.035,50),max:105+Math.min(t*.035,50),speed:range(36,48)+Math.min(t/35,isSurvival()?12:18),phase:range(0,7),heading:range(0,Math.PI*2),state:'roam',suspicion:0,memory:0,lastX:x,lastY:y,roamTime:range(1,4),hit:0})}
...
if(z.state==='roam'){z.roamTime-=dt;if(z.roamTime<=0){z.heading+=range(-1.8,1.8);z.roamTime=range(1.5,4)}}else{let targetDistance=Math.hypot(z.lastX-z.x,z.lastY-z.y);if(targetDistance>14)z.heading=Math.atan2(z.lastY-z.y,z.lastX-z.x);else{z.heading+=dt*1.25;speed=0}}
